...
def load_model_and_tokenizer():
    ...
    if Path(config.QWEN_LORA_PATH).exists() and config.LoRA_ENABLE is True:
        logger.info(f"挂载 LoRA: {config.QWEN_LORA_PATH}")
        model = PeftModel.from_pretrained(model, config.QWEN_LORA_PATH)
